...
?>
<head>
    <title>VereinsManagerPro (Termin)</title>
	<meta charset="utf-8">
...
<body>
    <h1>VereinsManagerPro Doodle (Terminplaner)</h1>
        <?php
    ...
    <p><a href='https://www.cloud4b.space/VereinsManager/Doodle/doodle.php'>zurück zur Übersicht</a></p> 
        <table>
            <thead>
                <tr><th colspan='3'>Mitglied</th><th>Anmeldestatus</th><th>Bemerkungen</th>
        ...
        $result01 = mysqli_query($conn, $sql01);
                    while($row01 = $result01->fetch_assoc()) {
                        ...
                        echo "<td>" . $row01['KontaktVorname'] . "</td>";
                              $sql04 = "SELECT * FROM terminkontrolle WHERE KontrolleArt = 'Anmeldung' AND KontrolleMitgliedId = " . $row01['KontaktId'] . " AND KontrolleTerminId = " . $_GET['TerminId'];
        $result04 = mysqli_query($conn, $sql04);
        $row04 = $result04->fetch_assoc();
                        switch($row04['KontrolleWert']){
                            case 1:
                                echo "<td  style='text-align:center' class='OK'>";
                                break;
                            case 2:
                                echo "<td style='text-align:center' class='NOK'>";
                                break;
                            case 3:
                                echo "<td style='text-align:center' class='EVT'>";
                                break;
                            default:
                        ...
                        echo "<input type='hidden' name='einzeltermin' value='1'>";
